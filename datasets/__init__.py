...
class RASampler(Sampler):
    ...
    def __init__(self, dataset, num_replicas=None, rank=None, shuffle=False, seed=0, repetitions=3):
...
class InfiniteBatchSampler(Sampler):
    def __init__(self, dataset_len, batch_size, seed=0, filling=False, shuffle=False, drop_last=False):
...
class DistInfiniteBatchSampler(InfiniteBatchSampler):
    def __init__(self, world_size, rank, dataset_len, glb_batch_size, seed=0, repeated_aug=0, filling=False, shuffle=False):
...
#     root = data_dir
#
#     # 根据数据集名称创建对应的数据集对象（这里假设你的数据集创建逻辑类似之前提到的 edges2handbags 等情况）
...
#     )
#     # 创建训练集的 DataLoader，使用 DistributedSampler
...
#         shuffle=False
#     )
#     # 创建验证集的 DataLoader，使用 DistributedSampler
...
#             shuffle=False
#         )
#         # 创建测试集的 DataLoader，使用 DistributedSampler
