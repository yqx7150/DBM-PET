...
class TrainLoop:
    ...
    def _load_and_sync_parameters(self):                    #从检查点加载模型参数，并在分布式训练环境中同步各个进程
    ...
    def _load_ema_parameters(self, rate):               #加载 EMA 参数，确保在分布式训练环境中各进程的一致性
    ...
    def _load_optimizer_state(self):                    #加载优化器的状态，以支持从检查点恢复训练过程
        main_checkpoint = find_resume_checkpoint() or self.resume_checkpoint
    ...
    def run_loop(self):
        # 训练过程的主循环，处理数据批次，执行训练和测试步骤，并定期保存状态和记录日志
        while True:
            for batch1, batch2, batch3, cond1, cond2, cond3, _ in self.data:
                # 检查是否达到训练步骤上限，若已达到则保存模型并退出
                ...
                # 预处理所有批次
                batch1 = self.preprocess(batch1)
                batch2 = self.preprocess(batch2)
                batch3 = self.preprocess(batch3)


                # 数据增强
                if self.augment is not None:
                    batch1, _ = self.augment(batch1)
                    batch2, _ = self.augment(batch2)
                    batch3, _ = self.augment(batch3)

                # 处理条件数据
                cond1 = self._process_condition(cond1)
                cond2 = self._process_condition(cond2)
                cond3 = self._process_condition(cond3)

                # 在所有批次完成后才递增 step
                took_step = False
                if self.run_step(batch1, cond1):
                    if self.run_step(batch1, cond2):
                        if self.run_step(batch1, cond3):
                ...
                if took_step and self.step % self.save_interval == 0:
                    ...
                    # 执行测试步骤
                    test_batch1, test_batch2, test_batch3, test_cond1, test_cond2, test_batch3, _ = next(
                        iter(self.test_data))

                    # 预处理测试批次
                    test_batch1 = self.preprocess(test_batch1)
                    test_batch2 = self.preprocess(test_batch2)
                    test_batch3 = self.preprocess(test_batch3)

                    # 处理测试条件
                    test_cond1 = self._process_condition(test_cond1)
                    test_cond2 = self._process_condition(test_cond2)
                    test_cond3 = self._process_condition(test_cond3)

                    # 执行测试步骤
                    self.run_test_step(test_batch1, test_cond1)
                    self.run_test_step(test_batch2, test_cond2)
                    self.run_test_step(test_batch3, test_cond3)
    ...
    def _process_condition(self, cond):
        """处理条件数据的方法"""
