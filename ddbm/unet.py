...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, xT=None, y=None):
        ...
        #     print(f"Shape of h[{i}]: {tensor.shape}")
        # for i, tensor in enumerate(emb):
        #     print(f"Shape of emb[{i}]: {tensor.shape}")
        # # print('hs', hs[0].shape)  # 查看列表中的第一个元素的形状
        # # print('hs list length:', len(hs))  # 打印列表长度
        # print(f"Shape of h: {h.shape}")
        # print(f"Shape of hs[-1]: {hs[-1].shape}")
        # for i, tensor in enumerate(hs):
        #     print(f"Shape of hs[{i}]: {tensor.shape}")
