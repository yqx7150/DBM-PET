...
def load_images_from_directory(directory):
    ...
    for filename in os.listdir(directory):
        if filename.endswith('.png'):
            ...
            image = Image.open(file_path).convert('RGB')  # 转换为RGB格式
            images.append(np.array(image))  # 转换为NumPy数组
    return np.array(images)  # 返回所有图像组成的NumPy数组
...
def get_ssim_lpips(args):
    ...
    ref_batch = load_images_from_directory(args.ref_batch)
    sample_batch = load_images_from_directory(args.sample_batch)
    print(f'Reference batch shape: {ref_batch.shape}')
    print(f'Sample batch shape: {sample_batch.shape}')
    assert ref_batch.shape == sample_batch.shape
    # 转换为Tensor并调整维度
    # transform = transforms.ToTensor()
    # ref_batch_tensor = transform(ref_batch).permute(0, 2, 1)  # (B, C, H, W)
    # sample_batch_tensor = transform(sample_batch).permute(0, 2, 1)  # (B, C, H, W)
    #
    # ds = torch.utils.data.TensorDataset(ref_batch_tensor, sample_batch_tensor)
    ...
    mse_all = []
    psnr_all = []
    for i, (ref, sample) in enumerate((dataloader)):
        # ref = ref.to('cuda').float() / 255 * 2 - 1
        ref = ref.to('cuda').float()
        # sample = sample.to('cuda').float() / 255 * 2 - 1
        sample = sample.to('cuda').float()
        ...
        with torch.no_grad():
            ...
            print(f"Batch {i+1} - SSIM: {ssim_score.item():.4f}, LPIPS: {lpips_score.item():.4f}, MSE: {mse_score.item():.4f}, PSNR: {psnr_score.item():.4f}")
